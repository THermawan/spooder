...
$resultFirst = pg_query_params($dbconn, 'SELECT username FROM users WHERE email = $1', array("$email"));

if(pg_fetch_row($resultFirst)){
	echo "invalid\n";
	exit;
}

$resultSecond = pg_query_params($dbconn, 'SELECT username FROM users WHERE username = $1', array("$username"));

if(pg_fetch_row($resultSecond)){
	echo "invalid\n";
	exit;
}

$result = pg_query_params($dbconn, 'INSERT INTO users (email, username, password, avatar) VALUES ($1, $2, $3, $4)', array("$email", "$username", "$password", "$avatar"));
