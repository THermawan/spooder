...
$result = pg_query_params($dbconn, 'UPDATE users SET avatar = $1 WHERE email = $2', array("$avatar", "$email"));
...
$result2 = pg_query_params($dbconn, 'SELECT avatar FROM users WHERE email = $1', array("$email"));
